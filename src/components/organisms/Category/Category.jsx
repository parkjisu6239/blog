    ...
    <article className={categoryCss}>
      ...
      <section className={postListWrapperCss}>
        ...
        <div className={postListCss}>
          {posts.slice((page-1)*5, page*5).map((post, idx) => <PostThumbnail category={category} post={post} key={idx}/>)}
        </div>
        <Pagination count={Math.ceil(posts.length/5)} page={page} onChange={changePage} shape="rounded"/>
      </section>
    </article>
  )
}
